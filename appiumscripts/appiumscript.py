...
def run_notification_script(collection, automatedMessagesCollection, utilsCollection):
    ...
    for doc in docs:
        ...
        if (timedelta(minutes=5) <= diff <= timedelta(minutes=6)):
            send_notification(utilsCollection, "Sending Notification to " + doc["name"] + " in 5 minutes!",
                              doc["aiMessageToSend"])
